...
Each DAX RX channel (1–8) has an independent gain control in the DAX Audio applet. Adjusting these lets you match the audio level delivered to digital mode software or other applications receiving DAX audio.
...
2. Locate the row for the channel you want to adjust: **DAX 1:** through **DAX 8:**. The rows visible depend on the connected radio's slice capacity — a radio with fewer slices does not display dead gain sliders for channels beyond its capacity.
...
Each RX gain+meter is a combined level meter and gain slider. The background bar shows the smoothed post-fader signal level in real time. The vertical thumb line marks the current gain position. Dragging the thumb emits the new gain and persists it immediately. RX meter levels use exponential smoothing with a fast attack and slow decay, so level peaks appear quickly and fall more gradually.

Each slider has an accessibility name set to "DAX RX 1 gain", "DAX RX 2 gain", "DAX RX 3 gain" through "DAX RX 8 gain" respectively for screen reader support.
...
- Gain values are stored as two-decimal-place floats (for example, `0.75`). They are restored from `DaxRxGain1`–`DaxRxGain8` each time AetherSDR starts.
...
- Rows for DAX channels beyond the connected radio's slice capacity are hidden entirely. For example, a radio with 4 slices shows only DAX channels 1–4. TX gain remains available on all radios.
...
- **Some DAX channel rows are missing** — This is normal when the connected radio has fewer slices than the maximum. For example, a radio with 4 slices displays only DAX channels 1–4. The hidden rows have no effect on the remaining channels.
