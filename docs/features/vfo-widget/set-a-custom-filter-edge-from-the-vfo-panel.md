...
Click the VFO marker flag on the spectrum display for the slice you want to control. The VFO panel appears anchored to the flag.

If the VFO panel is collapsed (shows only a frequency strip), click anywhere on it to expand it.
...
|---|---|---|
| RX antenna button | Opens antenna selection menu for the receive antenna of this slice. Uses per-slice antenna list from the radio when available; falls back to the global antenna list if empty. Each menu item shows its internal antenna name as a tooltip and status tip. | — |
...
| Slice badge | Shows the slice letter (e.g., A, B) in a colored badge. Supports HTML rendering for the slice letter (#2606). Click to select this slice. | — |
