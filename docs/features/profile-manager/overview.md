...
- A **Profile name** text field at the top. Type a name here before saving or creating a new profile, or select an existing profile from the list to populate this field automatically. The **Save** (or **Create**) button is disabled when the name field is empty, preventing accidental clicks with no target.
...
- A result status line below the buttons. This line appears momentarily to confirm a successful save or to show an error message. It disappears when you start typing in the **Profile name** field or load a different profile.
...
When you click **Save** on the **Global** tab, the radio saves the current state under the name in the **Profile name** field, overwriting an existing profile if one with that name already exists. If the field is empty, the currently selected profile name is used instead. A status message appears confirming success or reporting an error.
...
The Auto-Save tab contains a single checkbox and a short description. When **Auto-save profile changes** is checked, changes to TX and microphone settings are written back to the active profile automatically. This state is read live from the radio model, not from local AppSettings. When unchecked, you must use **Save** or **Create** explicitly to preserve any changes.
...
| Profile name              | Text field                                                                                                                                                                                                                     | Name used when saving a new profile. Populated automatically when a profile is selected. Enables or disables the Save/Create button as text is typed or cleared. Typing clears any status result from a previous save. |
| Profile list              | List                                                                                                                                                                                                                           | All profiles for this category; active one highlighted.                                                                                                                                      |
| Load                      | Button                                                                                                                                                                                                                         | Loads the selected profile onto the radio. Disabled when nothing is selected. Clears the status line.                                                                                        |
| Save                      | Button                                                                                                                                                                                                                         | On the Global tab only. Saves the current radio state under the typed name, overwriting an existing profile if one with that name exists. Disabled when the name field is empty. Shows a status message on completion or error. |
| Create                    | Button                                                                                                                                                                                                                         | On the Transmit and Microphone tabs only. Creates a new profile under the typed name. Cannot overwrite an existing profile; if a name collision occurs, a dialog offers to enable Auto-Save. Disabled when the name field is empty. Shows a status message on completion or error. |
| Delete                    | Button                                                                                                                                                                                                                         | Deletes the selected profile after confirmation. Disabled when nothing is selected.                                                                                                          |
| Auto-save profile changes | When enabled, TX changes are written back to the active profile automatically. State is read live from the radio model's auto_save status, not from local AppSettings. Sends 'profile autosave on/off' to the radio on toggle. | New in v26.7.4: reads/writes radio model directly. No longer persisted via AutoSaveTransmitProfile AppSettings key.                                                                          |
...
- The status line after saving shows either a success message (e.g., "Profile saved.") or an error message (e.g., "Save timed out."). Typing in the name field clears this status so you always see fresh feedback.
- When loading a transmit profile, the radio uses the `profile tx load` command internally. This change is transparent and does not affect the user experience.
- Disabled buttons are visually dimmed so you can tell at a glance which actions are available.
