...
6. Read the **Temp** text for heatsink temperature. Click the temperature label to toggle between Celsius and Fahrenheit. The temperature label is a clickable button; the current unit is indicated by the °C or °F suffix.
...
10. Click the **Fan Speed** button to cycle through STANDARD, CONTEST, and BROADCAST modes. The button shows the current mode as "Fan: Std", "Fan: Contest", or "Fan: Bcast". The button is hidden until the first fan mode status arrives from a direct PGXL connection. Added in v26.6.3.
...
| Temp | PGXL heatsink temperature | > 80 °C | Clickable button toggles between Celsius and Fahrenheit. Shows "Temp: xx.x C" or "Temp: xx.x F". Setting is persisted across sessions. Added in v26.7.4 |
...
| Fan Speed | Cycle STANDARD / CONTEST / BROADCAST | — | Hidden until direct PGXL connection delivers fan mode. Shows "Fan: Std", "Fan: Contest", or "Fan: Bcast". Added in v26.6.3 |
...
- The **Temp** gauge uses three color zones: green below 55 °C, yellow from 55 °C to 80 °C, and red above 80 °C. When switching to Fahrenheit, the threshold equivalents are: green below 131 °F, yellow from 131 °F to 176 °F, and red above 176 °F.
