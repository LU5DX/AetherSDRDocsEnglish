...
| **10 MHz Reference Source:** | Combo box | Selects the oscillator reference source. Valid values: `Auto` \| `TCXO` \| `GPSDO` \| `External`. Options shown depend on installed hardware. Lock status (Locked / Unlocked) is displayed alongside the combo and updates live. |
