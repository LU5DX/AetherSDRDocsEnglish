...
|---------|------|-------|----------|
| Level | Meter | -40 to +10 dBFS (red > 0) | Shows microphone input peak level in dBFS. Suppressed to -150 when met_in_rx is off and not transmitting. Hidden when the radio does not expose a client-selectable mic input (capability-aware, v26.8.4). Hover over the gauge to see the exact value with one decimal (v26.7.4). |
...
| Mic source | Combo box | MIC, BAL, LINE, ACC, PC (plus any from micInputList()) | Selects microphone input source; calls TransmitModel::setMicSelection. When the radio does not expose a client-selectable mic input, this combo box is narrowed to a single "PC" entry with a tooltip explaining that the radio's input selection is made on the radio itself (v26.8.4). |
...
| DAX | Toggle | On/Off | Enables DAX as the TX audio source; calls TransmitModel::setDax. Hidden when the radio does not support DAX as a TX source (v26.8.4). |
...
## Capability-aware mic source and Level meter (v26.8.4)

In v26.8.4, the Phone sub-panel adapts to the radio's audio-input capabilities:

- **Mic source combo box**: When the radio does not expose a client-selectable mic input (for example, a radio that takes transmit audio from the network port only), the combo box is narrowed from the full FlexRadio connector list (MIC, BAL, LINE, ACC, PC) to a single "PC" entry. The combo box is disabled and shows a tooltip: "This radio takes transmit audio from this computer. Its own input selection is made on the radio." The model is updated to report "PC" so downstream tools (such as radiocert) do not warn about transmit audio capture.
- **Level meter**: Hidden entirely when the radio does not expose a client-selectable mic input, since the client cannot meaningfully measure or report a mic level it does not control.
- **DAX button**: Hidden when the radio does not support DAX as a TX source.
- These changes are driven by capability notifications and apply immediately when the radio's capabilities change.
...
- In v26.8.4, the CW mode detection uses a single `isCwMode()` helper (VoiceModeGate) instead of inline CW-mode string comparisons, so the applet correctly recognizes "CW" (Flex), "CWU" (Icom, HL2), and "CWL" (reverse-side) as CW modes.
- In v26.8.4, the rebuilt Mic source combo box suppresses the operator-intent signal path during rebuild, so updating the combo box does not accidentally trigger a radio command that would overwrite a valid selection.