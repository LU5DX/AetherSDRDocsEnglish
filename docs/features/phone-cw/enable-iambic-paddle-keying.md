...
| NOR/DX/DX+        | Slider                                                                                                                                                            | Three-position processor level; calls TransmitModel::setSpeechProcessorLevel.                                                                   |
...
| Monitor volume    | Slider                                                                                                                                                            | Set sideband monitor volume; calls TransmitModel::setMonGainSb.                                                                                 |
...
| ALC gauge (Phone)    | -20 .. 0 dBFS (fill from right) | Automatic level control — post-software-ALC SSB peak, read from MeterModel::swAlcChanged. Initialized to -20 dBFS on startup. |
...
- **The Level gauge does not appear after connecting with mic source set to PC** — In v26.5.3, the Level gauge is suppressed for all mic sources (including PC and RADE) when the Level Meter During Receive setting is off and the radio is not transmitting. If the gauge is missing during receive, enable **Level Meter During Receive** in the radio settings or begin transmitting. When transmitting, the Level gauge will appear regardless of the setting.
...
- **The ALC gauge reads meaningless values or does not respond** — In v26.5.1, the ALC meter was rewired from HWALC (RCA voltage) to the