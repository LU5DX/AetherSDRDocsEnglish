...
| Compression         | Speech compression amount in dB (Phone panel). Gated on the radio's interlock TRANSMITTING state and speech processor enable: reads 0 dB during RX (v0.9.7).                                                                                                                                                     | —                                                        |
...
| Compression gauge    | Speech compression amount in dB (reversed fill).                                                                                            | -25 to 0 dB             | Gated on the radio's interlock TRANSMITTING state and speech processor enable: reads 0 dB during RX (v0.9.7).                   |
| ALC gauge (Phone)    | Automatic level control — post-software-ALC SSB peak, read from MeterModel::swAlcChanged. Fills right-to-left: empty at -20 dBFS, full at 0 dBFS. | -20 to 0 dBFS (red > -3) | Rewired from HWALC (RCA voltage) to SW ALC meter in v26.5.1 (#2552). Mirrored by identical gauge on CW panel.                  |
| ALC gauge (CW)       | Mirror of the Phone-panel ALC gauge, identically scaled. Both read from MeterModel::swAlcChanged.                                            | -20 to 0 dBFS (red > -3) | Added in v26.5.1 (#2552) as part of the SW ALC meter split. Uses HGauge::setFillFromRight mode.                                 |
...
- **ALC gauge reads 0 or meaningless values** — This issue was resolved in v26.5.1 (#2552). The ALC gauges now read from the correct software ALC meter source. If you still see 0 during transmit, ensure the radio is in a mode that produces ALC (SSB or CW).
