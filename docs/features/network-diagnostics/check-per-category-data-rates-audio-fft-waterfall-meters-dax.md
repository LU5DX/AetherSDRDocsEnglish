...
The Network Diagnostics dialog provides a comprehensive live view of the network link to the radio. It features a multi-tab layout with an Overview dashboard with health cards and time-series charts, a Connection Details pane with labeled metrics (including the Adaptive Frame-Rate Throttle section), searchable Latency & Jitter / Stream Rates / Packet Loss / Audio trend graphs, an Application Logs page for tailing the log file filtered by category, per-stream RX audio diagnostics, and a TCI Clients page (build-gated).
...
| **Connection Details** | Scrollable grid with labeled values for Network Status, Incoming Stream Rates, Packet Loss, Audio Playback, and the Adaptive Frame-Rate Throttle subsection. Renamed from "Details" in the v26.8.4 tree-navigation rework. |
...
| **TCI Clients** | Lists connected TCI clients with a live traffic monitor (Pause/Save log/Clear), answer-command suppression controls, and per-client details. New in v26.8.4. Build-gated by HAVE_TCI / TCI support. |

## Adaptive Frame-Rate Throttle

The **Adaptive Frame-Rate Throttle** section under Connection Details shows the adaptive throttle's Current State, Pending Lift and Sessions This Run values. This feature reduces panadapter frame rates when latency or loss is detected. New in v26.8.4. Hidden until throttle data is available.

## Controls

| Control                                | Location                                                                                                                                                     | Description                                                                                                                                                                                       |
|----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Timeframe**                          | Top-right corner of the tab bar                                                                                                                              | Selects how far back the time-series charts display history. Default is **5 minutes**. Options: 1 minute, 5 minutes, 15 minutes, 1 hour, 1 day, 1 week. Hidden when the **Logs** tab is active or on the **TCI Clients** page. |
...
| **Navigation tree**                    | Left side of dialog                                                                                                                                          | Tree widget listing all diagnostic pages. Click any item to switch the main view to that section.                                                                               |
| **Read-Only Clipboard Copy**           | Connection Details page                                                                                                                                      | Copies the whole diagnostic summary.                                                                                                                                                             |
...
| **Latency (RTT)** | Current round-trip time. May show "not measured on this link" or "n/a" on transports that have no round trip to time. |
| **Max Latency (RTT)** | Highest RTT seen since connect. May show "not measured on this link" on transports that have no round trip to time. |
| **Audio / FFT / Waterfall / Meters / DAX rates** | Per-category ingress rate in kbps. May show "n/a" on transports that do not expose per-stream category stats. |
| **Total RX / Total TX** | Aggregate bytes per second in each direction. |
| **Audio / FFT / Waterfall / Meters / DAX drops** | Per-category dropped-packet counts and percentage. May show "n/a" on transports that do not expose per-stream category stats. |
| **RX Buffer Now / Peak** | Current and peak audio buffer fill in bytes and ms. |
| **Underruns (total / last sec)** | Audio underrun counters. |
| **Audio Arrival Gap / Max Arrival Gap** | Inter-packet arrival timing. May show "not measured on this link" before the first delivery-timing window closes. |
| **Network Jitter** | Smoothed jitter estimate of audio stream in ms. May show "not measured on this link" before the first delivery-timing window closes. |
| **Adaptive Frame-Rate Throttle** | Current State, Pending Lift and Sessions This Run values. New in v26.8.4. Hidden until throttle data is available. |
...
- A rate of 0 kbps for a category that should be active (for example, **Audio** while a slice is open) indicates the stream has stopped arriving. Check the **Status** indicator in the **Network Status** group on the **Connection Details** tab first.
- If a rate or latency indicator shows "n/a" or "not measured on this link", that diagnostic does not apply to the currently connected transport. This is distinct from a zero measurement, which would indicate an actual reading of zero.
...
- Zero drops in the **Packet Loss** group does not rule out jitter or late-arriving bursts. If audio is choppy but drops show zero, check the **Audio Playback** group on the **Connection Details** tab for underruns and jitter, or review the **Audio** tab for a time-series view of buffer fill and underruns/s.
...
- **All category rates show 0 kbps** — The radio is not streaming. Confirm the connection is active by checking **Status** and **Target Radio IP** in the **Network Status** group on the **Connection Details** tab. Reconnect via **Settings** > **Connect to Radio...** if needed.
