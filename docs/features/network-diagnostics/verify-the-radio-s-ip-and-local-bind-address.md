...
- **Logging**
  - Logs
- **TCI Clients** *(build-gated, shown only when TCI support is compiled in)*
...
| **Connection Details**    | Scrollable grid with labeled values for Network Status, Incoming Stream Rates, Packet Loss, and Audio Playback groups, plus the Adaptive Frame-Rate Throttle subsection. Renamed from "Details" in the v26.8.4 tree-navigation rework. Includes a Read-Only Clipboard Copy action to copy the whole diagnostic summary. |
...
| **Logs**                  | Live tail of the AetherSDR log file, filtered by category checkboxes. Syntax-highlighted by log level and category name. Timeframe selector is hidden while this page is active.                   |
| **TCI Clients**           | Lists connected TCI clients with a live traffic monitor (Pause/Save log/Clear), answer-command suppression controls, and per-client details. New in v26.8.4. Build-gated by HAVE_TCI / TCI support. Timeframe selector is hidden on this page. |

## Verify the radio's IP and local bind address

1. Open the **Network Diagnostics** dialog as described above.
2. In the navigation tree, click **Connection Details**.
...
## Review the Adaptive Frame-Rate Throttle

The **Connection Details** page includes an **Adaptive Frame-Rate Throttle** section that shows:

- **Current State** — Whether throttling is currently active.
- **Pending Lift** — Whether a lift of throttling is pending.
- **Sessions This Run** — Number of throttle sessions since connect.

This throttle reduces panadapter frame rates when latency or packet loss is detected on the link. The section is hidden until throttle data becomes available.

## Control the chart timeframe

The **Timeframe** combo box in the top-right corner sets how far back all time-series charts display history. It is hidden while the **Logs** or **TCI Clients** page is active.
...
## Use the TCI Clients page *(build-gated)*

> **Note:** This page appears only in builds compiled with TCI support (`HAVE_TCI`).

1. In the navigation tree, click **TCI Clients**.
2. The page lists all connected TCI clients.
3. For each client, review per-client details such as connection state and traffic activity.
4. Use the traffic monitor controls:
   - **Pause** — freeze the live traffic display.
   - **Save log** — export the captured traffic log to a file.
   - **Clear** — reset the traffic monitor.
5. Use the answer-command suppression controls to prevent AetherSDR from responding to specific TCI commands, if needed.

> **Note:** The **Timeframe** selector is hidden while this page is active.
...
| **Latency (RTT)**                       | Current round-trip time. Displays "not measured on this link" if the transport has no round trip to time. |
| **Max Latency (RTT)**                   | Highest RTT seen since connect. Displays "not measured on this link" if the transport has no round trip to time. |
| **Audio / FFT / Waterfall / Meters / DAX rates** | Per-category ingress rate in kbps. Displays "n/a" on transports that do not expose per-stream category statistics. |
| **Total RX / Total TX**                 | Aggregate bytes per second in each direction.                                           |
| **Audio / FFT / Waterfall / Meters / DAX drops** | Per-category dropped-packet counts and percentage. Displays "n/a" on transports that do not expose per-stream category statistics. |
| **RX Buffer Now / Peak**                | Current and peak audio buffer fill in bytes and ms.                                     |
| **Underruns (total / last sec)**        | Audio underrun counters.                                                                |
| **Audio Arrival Gap / Max Arrival Gap** | Inter-packet arrival timing. Displays "not measured on this link" before the first timing window closes or if the transport reports no timing. |
| **Network Jitter**                      | Smoothed jitter estimate of the audio stream in ms. Displays "not measured on this link" before the first timing window closes or if the transport reports no timing. |
...
|------------------------------|----------------|------------|------------------------------------------------------------------------------------------------------------------------------------|
| **Timeframe**                | Combo box      | 5 minutes  | Selects how far back the time-series charts display history. Hidden when the Logs or TCI Clients page is active.                   |
...
- On transports with no round trip to time, the **Latency (RTT)** and **Max Latency (RTT)** indicators display "not measured on this link" rather than a misleading "< 1 ms" value. Where per-stream category statistics are not available, rate and drop fields display "n/a" rather than zeroed rows.
...
- **Audio stream shows high late packet count** — Check the **Latency** page for jitter spikes and the **Packet Loss** page for Audio stream drops. Consider increasing the audio buffer size in audio