...
6. On the TX side only, the RN2 toggle is located below the output level meter. When enabled, it applies the RNNoise neural denoiser on the mic input before the DSP chain. RN2 is only active in voice modes — digital modes (RADE, DAX, RTTY, FT8, FDV, CW) bypass this stage. The setting is persisted via AudioEngine.
