...
| Control        | Behavior description                                                                                                                                                                                                                                                                                                                                                                          | Default   |
|----------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
...
| Drive          | Linear mapping. Pushes more signal into the tube stage. Setting key: `ClientTubeTxDriveDb` / `ClientTubeRxDriveDb`. Label 'X.XX dB'.                                                                                                                                                                                                                                                          | 0.00 dB   |
| Tone           | Linear mapping. Negative values darken, positive brighten the saturated signal. Setting key: `ClientTubeTxTone` / `ClientTubeRxTone`. Label 'X.XX'.                                                                                                                                                                                                                                            | 0.00      |
| A              | Selects tube character Model A. Exclusive with B and C. Amber when checked. Setting key: `ClientTubeTxModel` / `ClientTubeRxModel` (stored as integer 0/1/2).                                                                                                                                                                                                                                 | checked   |
| B              | Selects tube character Model B. Exclusive with A and C. Amber when checked. Setting key: `ClientTubeTxModel` / `ClientTubeRxModel` (stored as integer 0/1/2).                                                                                                                                                                                                                                 | unchecked |
| C              | Selects tube character Model C. Exclusive with A and B. Amber when checked. Setting key: `ClientTubeTxModel` / `ClientTubeRxModel` (stored as integer 0/1/2).                                                                                                                                                                                                                                 | unchecked |
| Bias           | Linear mapping. Shifts the operating point on the transfer curve, changing the harmonic mix. Label displayed as percentage. Setting key: `ClientTubeTxBias` / `ClientTubeRxBias`.                                                                                                                                                                                                             | 0 %       |
| Output         | Linear mapping. Post-tube make-up / trim gain. Label 'X.XX dB'. Setting key: `ClientTubeTxOutputDb` / `ClientTubeRxOutputDb`.                                                                                                                                                                                                                                                                 | 0.00 dB   |
| Dry/Wet        | Linear mapping. Dry / wet blend (100 % = fully saturated signal). Label displayed as percentage. Setting key: `ClientTubeTxDryWet` / `ClientTubeRxDryWet`.                                                                                                                                                                                                                                    | 100 %     |
| Envelope       | Linear mapping (-1.0 to +1.0). Positive values increase drive on transients (the tube gets hotter on loud peaks); negative values reduce it, compressing harmonics dynamically. Label displayed as signed percentage. Setting key: `ClientTubeTxEnvelope` / `ClientTubeRxEnvelope`.                                                                                                            | 0 %       |
| Attack         | Exponential mapping (0.1 * 300^n). Sets how quickly the envelope follower responds to rising levels when Envelope ≠ 0. Label 'X.XX ms' below 10 ms, 'X.X ms' above. Setting key: `ClientTubeTxAttackMs` / `ClientTubeRxAttackMs`.                                                                                                                                                             | 5.00 ms   |
| Release        | Exponential mapping (10 * 50^n). Sets how quickly the envelope follower recovers after levels drop when Envelope ≠ 0. Label 'X.XX ms' below 100 ms, 'X.X ms' above. Setting key: `ClientTubeTxReleaseMs` / `ClientTubeRxReleaseMs`.                                                                                                                                                           | 35.00 ms  |
