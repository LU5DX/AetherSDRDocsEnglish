...
| Ratio                  | 2.0       | 1.0 to 10.0       | Linear mapping. Higher ratios give a harder, more gate-like cut; lower ratios act as a soft downward expander.                                                                                                                                                        |
| Return                 | 2.0 dB    | 0.0 to 20.0 dB    | Linear mapping (n * 20). Sets the hysteresis deadband: the gate opens above Thresh and doesn't close again until the input drops below Thresh − Return, preventing rapid chatter near the threshold. The curve widget draws a soft-cyan vertical band between (Thresh − Return) and Thresh to make the sticky zone visible. |
...
### Animation performance (v26.6.3)

In v26.6.3, the animation smoothing logic was improved to reduce unnecessary repaints. The Transfer curve widget and Gain-reduction bar now only repaint when the smoothed value actually changes or when the animation settles, rather than repainting on every animation tick. This reduces CPU overhead during live animation while maintaining the same visual responsiveness.
