...
|-------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| **Spots:** toggle button      | Master toggle for DX spot display. Click to toggle between enabled and disabled states. The button label updates to "Enabled" or "Disabled" based on the current state. Saved to `IsSpotsEnabled`.                                                                                                    | Enabled |
| **Memories:** toggle button   | Toggles memory channel overlays on the panadapter. Click to toggle between enabled and disabled states. The button label updates to "Enabled" or "Disabled" based on the current state. Saved to `IsMemorySpotsEnabled`. Setting key changed from `IsMemoriesShownOnPanadapter` in v0.9.7.              | Disabled |
...
| **Override Colors:** toggle button | Forces a single text color for all spots. Click to toggle between enabled and disabled states. The button label updates to "Enabled" or "Disabled" based on the current state. Saved to `IsSpotsOverrideColorsEnabled`.                                                                                | Disabled |
...
| **Override Background: Enabled** toggle button | Draws a background under spot text. Click to toggle between enabled and disabled states. The button label updates to "Enabled" or "Disabled" based on the current state. Saved to `IsSpotsOverrideBackgroundColorsEnabled`.                                                                             | Enabled |
| **Override Background: Auto** toggle button | Auto-picks background color for contrast. Click to toggle between enabled and disabled states. The button always displays "Auto". Saved to `IsSpotsOverrideToAutoBackgroundColorEnabled`.                                                                                                            | Enabled |
...
| **Spot Lines:** toggle button | Draws vertical lines from the spectrum baseline up to each spot label. Click to toggle between enabled and disabled states. The button label updates to "Enabled" or "Disabled" based on the current state. Disable during contests to reduce visual clutter. Saved to `IsSpotsLinesEnabled`. Added in v0.9.7 (#2349). | Enabled |
...
- Toggle buttons now display either "Enabled" or "Disabled" based on their current state, making the status visible at a glance. Previously, buttons always showed "Enabled" regardless of state.
