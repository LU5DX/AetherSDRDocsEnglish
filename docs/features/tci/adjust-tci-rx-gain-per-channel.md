...
Each meter/slider has an accessible name set for screen reader compatibility. The RX gain sliders are named "TCI RX 1 gain" through "TCI RX 4 gain", and the TX gain slider is named "TCI TX gain". The Enable button has an accessible name of "TCI server enable" with a description of "Start or stop the TCI server".
