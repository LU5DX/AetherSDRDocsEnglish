...
- Know which memories you want to remove. Use Search:, Profile:, or the memory table to narrow the list first if needed.
...
3. Confirm your selection by making sure the right number of rows are selected:
   - The selection count indicator in the lower-right area of the dialog shows `<N> of <M> selected` (where N is the number of selected rows and M is the total number of rows).
   - The button label changes to "Remove Selected" when more than one row is selected.
4. Click Remove once, not twice. The button label changes to "Remove Selected" when more than one row is selected. If you click it twice in a row, the first click removes the memories and the second click removes nothing (because the selection is cleared).
...
- The dialog has a frameless title bar with a grip glyph on the left. Double-click the title bar to toggle maximize/restore.
