...
- If the applet is not visible, open the Aetherial Audio Channel Strip, which hosts the de-esser controls directly. The floating StripDeEssPanel (accessible through the Channel Strip) includes the Slope button; the docked Aetherial De-Esser applet omits the Attack, Release, and Slope knobs/buttons.
...
2. Locate the **Q** knob in the six-knob tuning row (Freq, Q, Thresh, Amount, Attack, Release). Note: In the docked ClientDeEssApplet, only Freq, Q, Thresh, and Amount are present; Attack and Release are omitted.
...
| **Attack**               | 1.0 ms (present in the Channel Strip StripDeEssPanel RX and TX only; omitted from the docked ClientDeEssApplet)                                                                                                            | 0.1 to 30.0 ms                                                                                                                                                                                                                                                  |
| **Release**              | 100 ms (present in the Channel Strip StripDeEssPanel RX and TX only; omitted from the docked ClientDeEssApplet)                                                                                                            | 10.0 to 500.0 ms                                                                                                                                                                                                                                                |
| Sidechain response curve | —                                                                                                                                                                                                                          | —                                                                                                                                                                                                                                                               |
| Gain-reduction bar       | —                                                                                                                                                                                                                          | 0 to 24 dB GR                                                                                                                                                                                                                                                   |
| **Slope**                | 24 dB/oct (2 stages). Cycles the sidechain bandpass cascade count. Each stage adds 12 dB/oct of rolloff outside the sibilant band. Higher slope = narrower effective notch, less mid-band collateral on Ess-heavy phrases. | 12 / 24 / 36 / 48 dB/oct (1 to 4 stages). Present as a push button labeled "N dB/oct" in the floating StripDeEssPanel only, anchored below the Thresh knob. Present for both TX and RX paths. Persisted as ClientDeEssTxSlopeStages / ClientDeEssRxSlopeStages. |
