...
- **Signal History color swatches:** Click to open a color picker for the voice signal markers (default gold `#FFC800`) and QRM markers (default red `#FF0000`). Saved to `SHistoryColorSignals` and `SHistoryColorQrm` respectively.
- **Snap to Step:** Toggle (default Disabled) that rounds S-History click-to-tune to the nearest multiple of the active slice's step size, hiding the small carrier offset. Saved to `SHistorySnapToStep`.

## DXCC Coloring (updated in v26.5.1