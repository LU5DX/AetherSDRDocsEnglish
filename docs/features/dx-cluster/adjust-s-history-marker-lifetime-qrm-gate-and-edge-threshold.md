...
1. Under **Signal History**, adjust **Marker Lifetime:** by dragging the slider. Inactive markers are removed after this many seconds (range 15–300 sec, default 60).

2. Adjust **QRM Gate:** — the seconds a narrow carrier or wideband signal must persist before the system classifies it as QRM (range 3–30 sec, default 6).

3. Adjust **Edge Threshold:** — the dB level above the noise floor for the slope-edge walk that refines a marker’s carrier-side edge. A lower value hugs the carrier more tightly but is more noise-sensitive (range 1.0–10.0 dB, default 3.0 dB).
...
| Control | Label on slider | Default | Valid range | Setting key |
|---|---|---|---|---|
| **Marker Lifetime:** | `15 sec` – `300 sec` | 60 | 15–300 sec | `SHistoryLifetimeS` |
| **QRM Gate:** | `3 sec` – `30 sec` | 6 | 3–30 sec | `SHistoryQrmGateS` |
...
- **Markers disappear too quickly** — Increase **Marker Lifetime:** to a higher value.
- **QRM markers appear on weak signals that aren’t really interference** — Raise **QRM Gate:** so the system requires a longer hold time before classifying the signal as QRM.
- **Marker edges wobble or jump** — Increase **Edge Threshold:** slightly (e.g., from 3.0 to 4.0 dB) to reduce noise sensitivity.
