...
| **QRM Gate** (slider) | `SHistoryQrmGateS` | 6 s | How long a narrow carrier or wideband signal must persist before being classified as QRM. Range: 3–30 s. |
| **Edge Threshold** (slider) | `SHistorySoftEdgeDb` | 3.0 dB | Threshold above noise floor for the slope edge walk that refines the S-History carrier-side edge. Range: 1.0–10.0 dB. Lower = closer to carrier but more noise-sensitive. |
