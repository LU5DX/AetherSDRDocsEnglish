...
| **DXCC Colors:**              | Colors spots by worked/confirmed/needed DXCC status. The button always shows "Enabled".                                                                                | `IsDxccColoringEnabled` (changed from `DxccColoringEnabled` in v26.5.1)                                            |
...
| **Snap to Step:**             | Rounds S-History click-to-tune to the nearest multiple of the active slice's step size. The button always shows "Enabled". Default: Disabled.                         | `SHistorySnapToStep` (new in v26.5.1)                                                                              |
...
The table can be sorted by clicking any column header. Both **Time** and **Freq** columns sort numerically using the underlying spot data, so clicking **Time** returns the list to newest-first order even after sorting by frequency.
...
| **Grid Square:** | Maidenhead grid square sent to the FreeDV Reporter map. The field is read-only while