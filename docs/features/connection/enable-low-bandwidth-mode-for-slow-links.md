...
- Enable **Use low bandwidth mode** before initiating the connection. This setting is negotiated at connect time.
- The local radio list now shows up to 240 pixels tall and scrolls internally. On small displays (e.g. a 1024x600 panel), the list no longer grows past the Connect button — remaining radios are always reachable via the vertical scrollbar.
- Right-click any discovered radio in the local list to set a custom nickname without connecting first. Non-Flex radios (HL2, simulator, etc.) store the nickname client-side by serial number; Flex radios do not offer this option to avoid conflicting with the on-radio name set in Radio Setup.
