...
- GPU acceleration requires a compatible NVIDIA GPU (with CUDA) or Apple Silicon Mac (with Metal) and sufficient VRAM. On hosts without a compatible GPU, the engine runs in CPU mode.
...
3. **Monitor the transcript** in real time as decoded text scrolls in the read-only **Transcript** field. Text is color-coded by confidence:
...
5. **Fine-tune performance** by clicking the **Settings button** (gear icon) to open the modeless Copy Assist settings dialog, where you can adjust:
   - **Model tier** – choose `tiny`, `base`, `small`, or `medium` (larger = more accurate but slower and uses more VRAM/RAM)
   - **Compute device** – select `GPU (CUDA/Metal)` for faster inference with GPU or `CPU` for universal compatibility. GPU availability is probed asynchronously on startup; the engine uses the platform default until the probe completes (on macOS the first GPU query can take several seconds while the Metal shader library compiles).
6. **Clear the buffer** at any time by clicking the **Clear** button. This also resets the whisper decode context so the next transcription starts from a clean prompt.

> **Note:** The **Clear** button and the transcript's internal clear both flush the carried decode context on the whisper backend, ensuring the fresh display starts with no stale prompt state.
...
| **Transcript** | read-only text field | — | Scrolling text display of decoded speech. Color-coded by confidence: green (high), yellow (medium), red (low). Includes a Clear button that empties the buffer and resets decode context. |
...
| **Clear** | push button | — | Clears the current transcript buffer and resets the whisper decode context. |

## Engine status indicator

The Copy Assist panel shows the current state of the speech-to-text engine:

| State | Meaning |
|---|---|
| Idle | Engine is not running. |
| Downloading model | The selected model file is being downloaded on first use. |
| Loading model | The model file is being loaded into memory. |
| Listening | Engine is actively decoding audio from the active slice. |
| Error | The engine encountered a problem (e.g., model download failure, backend unavailable). |
...
- An explicit compute-device choice is remembered and takes priority over the automatic GPU detection. If you later change hardware, clear the saved preference by selecting a different device or removing the setting via the configuration file.
