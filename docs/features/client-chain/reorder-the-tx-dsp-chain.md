...
| **BYPASS**                                                        | Toggle button | Disables every stage on the currently shown chain side at once. When checked, snapshots the currently-enabled stages on the active side (TX or RX) and disables all of them. Unchecked: re-enables just the stages that were on before. TX and RX maintain separate snapshots. The scope is global (per audio engine), not per-profile — the button stays pressed across Channel Strip profile switches.                                                                                                                                                             |
...
- The **BYPASS** button disables every stage in the selected chain.
...
- The ADSP bypass tile handles the NVAFX module (labeled "NVAFX") in the same manner as other noise reduction modules. When bypassing, NVAFX is disabled if active; when restoring, the saved state applies NVAFX if it was previously the active module.
