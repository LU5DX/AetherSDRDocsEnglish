...
1. In the Aetherial Audio Chain header row, confirm the **TX** button is selected. If not, click **TX**.
...
- **BYPASS button shows wrong state after switching tabs** — In v26.6.1, the BYPASS button correctly reads the engine's bypass state for the active side. Switching tabs now triggers the correct engine query: `isTxBypassed()` for TX mode and `isRxBypassed()` for RX mode.
- **BYPASS button appears to reset when changing Channel Strip profiles** — This is expected behaviour. The BYPASS state is global per audio engine, not stored per profile. The button stays pressed regardless of which profile is loaded.
- **ADSP tile restores DNR/NR2 instead of BNR** — In v26.7.4, the snapshot restoration on ADSP tile un-bypass uses NvAfx (previously BNR). If the prior noise reducer state included BNR, it will be restored as NvAfx instead.
