...
| **BYPASS**                                                                                | Toggle button | Disables every stage in the selected chain (including RN2). Click again to restore the stages that were on before. Scope is global (per audio engine), not per-profile — the button stays pressed across Channel Strip profile switches. TX and RX maintain separate snapshots — the visual checked state tracks the side currently shown.                                             |
...
- The **BYPASS** button operates globally on the audio engine, not per-profile. It remains pressed even when you switch Channel Strip profiles using the TX editor.
...
- Colours in the chain (backgrounds, borders, LEDs) now follow the active theme rather than being hard-coded, so the chain adapts to your chosen visual style.
...
- **BYPASS button state seems inconsistent across profiles** — The BYPASS is a global audio engine setting, not per-profile. It will remain engaged even when you switch Channel Strip profiles, which is by design.
