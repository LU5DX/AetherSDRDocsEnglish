...
3. Read the three tiles from left to right:
   ...
   - **ADSP** — mirrors which client-side noise reducer is currently active. The label rotates to the active module's short name (for example, `NR2`, `NR4`, `BNR`, `MNR`, `DFNR`, or `RN2`). When no noise reducer is on, the tile label falls back to `ADSP`. Single-click bypasses the entire NR cluster with an in-memory snapshot; single-click again restores the prior NR state. Double-click opens the AetherDSP Settings dialog.
...
| **ADSP**                                                                        | Toggle button / indicator | Label rotates to the active noise reducer's short name (`NR2`, `NR4`, `BNR`, `MNR`, `DFNR`, or `RN2`); shows `ADSP` when none is active. Single-click bypasses the entire NR cluster; single-click again restores the prior NR state. Double-click opens AetherDSP Settings. Greens when a noise reducer is on. The `BNR` module has been replaced by `NV AFX` internally — the tile label still shows `BNR` if that module was active when bypassed. |
...
- The ADSP tile now controls the `NV AFX` module in place of the legacy `BNR` module. The tile label still shows `BNR` if that module was active when the snapshot was taken, and restores to `NV AFX` on un-bypass.
