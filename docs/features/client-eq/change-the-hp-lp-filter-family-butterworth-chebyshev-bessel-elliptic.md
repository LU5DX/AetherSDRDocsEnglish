...
| Smoothing                           | Off (1/96)                                                                                                                                                                                                                                                                                                                                                                      | Off (1/96) \                                                                                                                                                                                                                              |
| Peak Hold                           | unchecked                                                                                                                                                                                                                                                                                                                                                                       | checked / unchecked                                                                                                                                                                                                                       |
...
**Output Fader** — Vertical combined fader + level meter on the right edge of the floating editor. Drag to set post-EQ master gain; scroll wheel adjusts in 0.5 dB steps; double-click resets to 0 dB. The level bar behind the handle shows the smoothed post-EQ peak in real time with the same green-amber-red gradient as the Tube level meter. Click the dB readout at the bottom to enter a value directly via keyboard. Persisted separately per path: ClientEqTxMasterGain / ClientEqRxMasterGain. Tooltip: 'Output gain (dB). Drag to set, wheel for fine step, double-click to reset to 0 dB.' Gain range is linear 0.0 to ~4.0; the scale labels run from -40 to 0 dB. Located in the floating editor only — not in the docked applet tile.

**Reference curve** — Overlays a semi-transparent amber target curve on the EQ canvas, showing the frequency response of a classic microphone or recommended speech contour. Use this visual target as a guide when shaping your parametric EQ bands. Available in the floating editor header strip. Options: Off, AT&T 1959 (Bell Labs optimum transmission response for speech, +5 dB peak at 2.5 kHz), Heil DX (Bob Heil's recommended pile-up talk power curve, +6 dB at 2.7 kHz), Astatic D-104 (classic crystal lollipop mic response, peaky presence around 3 kHz), Shure 444 (classic broadcast desk mic, gentler presence boost), Heil HC-5 (modern dynamic SSB mic target, +6 dB at 2.5 kHz). The curve is for visual reference only — it does not apply any filtering to the audio. Stored in settings key `ClientEqTxReferenceCurve` / `ClientEqRxReferenceCurve`.
...
The EQ canvas caches its background grid and response curve as retained layers to keep the 25 Hz analyzer redraws efficient. In v26.8.4, the cache is bounded to roughly 32 MB per layer; on unusually large canvases (for example, a 4K editor window maximised on a high-DPI display), the affected layer falls back to direct drawing, which preserves full fidelity at a small performance cost. You do not need to do anything — this happens automatically.
...
- Use the **Reference curve** to visually match your parametric EQ to a classic microphone response. Select from the dropdown in the editor header strip. The overlay does not affect audio — it is a visual guide only.
- The reference curve is stored separately for TX and RX. Change it independently for each path.
...
- **The canvas appears to redraw slowly on a very large display** — On unusually large canvases (such as a maximised 4K editor window at high DPI), the EQ canvas may fall back to direct drawing to preserve full fidelity. If this causes perceptible lag, reduce the editor window size or lower the display scaling.
