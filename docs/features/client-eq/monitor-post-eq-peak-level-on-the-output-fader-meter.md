...
| Numeric value edit                  | Displays the current gain in dB (e.g. "+0.0 dB"). When you click on it, it becomes an inline QLineEdit showing a bare number for typing. Press Enter or click elsewhere to commit the typed value within the −36.0 to +12.0 dB range. Press Escape to cancel and revert to the current value. The editor shows a cyan focus border (1px `#00b4d8`) to indicate edit mode.       | −36.0 to +12.0 dB (clamped on commit). Accepts numeric input with optional decimal point and sign (+/-).                                                                                                                         |
...
- Click the numeric display at the bottom of the fader to type an exact dB value. The field shows a bare number when focused; press Enter or click elsewhere to commit. The value is clamped to the −36.0 to +12.0 dB range.
