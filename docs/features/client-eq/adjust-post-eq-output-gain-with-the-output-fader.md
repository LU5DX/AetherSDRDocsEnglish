...
| Smoothing                           | Off (1/96). Applies fractional-octave power-averaging to the analyzer trace for display — does not affect EQ math. Lower fraction = smoother (1/3 is most smoothed; 1/96 is effectively off). Shared between TX and RX editors.                                                                                                                                                 | Off (1/96)                                                                                                                                                                                                                                |
| Peak Hold                           | Toggle button, unchecked. When checked, the per-bin peak-hold trace in the analyzer stops decaying — every frequency's highest observed level is held until the button is toggled off. Amber background when checked.                                                                                                                                                           | Located in the editor header strip (floating editor only).                                                                                                                                                                                |
...
| Preset          | Source                                                                                                                                 |
|-----------------|----------------------------------------------------------------------------------------------------------------------------------------|
| AT&T 1959       | AT&T 1959 "optimum transmission frequency response for speech" — canonical Bell Labs presence-peak target. Peak +5 dB at 2.5 kHz.      |
...
1. Open the floating editor for the path you want to adjust.
2. Locate the **Reference curve** selector in the editor header strip.
3. Click the selector and choose a preset from the list. The curve appears immediately on the canvas.
4. To remove the reference curve, select **Off** from the same selector.
...
- The analyzer display uses caching to keep rendering responsive. For very large canvas sizes (e.g., canvas dimensions exceeding approximately 32 MB of pixel data at your display's pixel ratio), the cache is bypassed and the canvas is drawn directly, which preserves fidelity. This is automatic and requires no action.
