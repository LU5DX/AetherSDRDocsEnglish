...
- The Smoothing control averages the displayed analyzer trace using fractional-octave power averaging. Use 1/3 for a smoother, easier-to-read trace when surveying broad response; use Off (1/96) when you need to see individual narrow peaks precisely. Smoothing does not affect how the EQ processes audio.
- The dashed yellow filter cutoff guide lines in the editor canvas show exactly where the radio is rolling off the signal. Drag a guide line directly in the editor to move the radio's passband in real time without leaving the EQ window.
