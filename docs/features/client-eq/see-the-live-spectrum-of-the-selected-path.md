...
- When the editor is in TX mode, the radio's TX filter low and high cutoffs are shown as dashed yellow guide lines on the canvas. These lines are not visible in RX mode. Dragging a cutoff line adjusts the corresponding TX or RX slice filter in real time.
...
- **Filter guide lines not showing on the TX path** — Guide lines are only drawn when TX filter cutoffs have been pushed to the editor. If no dashed yellow lines appear, verify that the radio's TX low/high filter cutoffs are being reported to the editor. Guide lines are suppressed for any cutoff value passed as 0. Guide lines do not appear on the RX path by design.
- **Dragging a cutoff guide line has no effect on the actual filter** — Cutoff drag requests are emitted as signals and applied by MainWindow. If the filter does not change when you drag a guide line, check that MainWindow is connected and that the correct path (TX or RX) is active.
