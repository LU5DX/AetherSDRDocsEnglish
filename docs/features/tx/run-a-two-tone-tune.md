...
7. When the test is complete, click **MOX** again to unkey the transmitter. The button returns to its unlit state with an amber border and text accent. Both the RF Pwr and SWR meters reset to their rest positions (0 W and 1.0) immediately on un-key.
...
| SWR        | Displays standing wave ratio at the exciter. When no SWR data is available (for example, during a pause in transmission), the meter parks at its 1.0 rest position rather than reading 0.0 or holding a stale value. | —                                                                                                                                                                       |
...
The **ATU** and **MEM** buttons are disabled when TGXL is in OPERATE mode, or when the connected radio has no antenna tuner (for example, a Hermes-Lite 2). When the button is disabled for either reason, hover over it to see a tooltip explaining why. The tooltip prioritizes the no-tuner explanation over the TGXL-OPERATE explanation, since a radio with no tuner makes the TGXL state irrelevant.
...
- **ATU and MEM buttons are grayed out** — Either the radio has no antenna tuner installed (e.g., a Hermes-Lite 2), or the external TGXL amplifier is in OPERATE mode. Hover over either button to see which reason applies. If the TGXL is in OPERATE mode, switch it out of OPERATE before using the internal tuner.
