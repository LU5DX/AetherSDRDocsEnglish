...
| Level bar | Smoothed peak fill | −60 dB (bottom) to 0 dB (top) | Uses fast-attack (alpha = 0.6) / slow-release (alpha = 0.08) ballistics. Gradient fill uses the theme token `color.meter.bar.fillGradient`; default colors: green (low) → lime → amber → red (top, above −3 dB). When the theme changes, the meter re-renders automatically. |
