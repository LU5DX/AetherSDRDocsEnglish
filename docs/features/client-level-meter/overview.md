...
| Level bar | Gradient-filled vertical bar showing the smoothed peak level. Fills from the bottom up, proportional to the current dB value. The fill gradient is themed via `color.meter.bar.fillGradient`. | −60 dB (bottom) to 0 dB (top) |
