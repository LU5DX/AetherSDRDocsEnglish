...
In v26.6.3, a WFM toggle button was added next to the Mode combo in the frequency row. This button enables the software FM demodulator, which processes the DAX IQ stream through a Hi-Fi Cable. WFM is not a radio mode and does not appear in the Mode combo — it is a client-side overlay that runs on the slice independently of the selected radio mode.

- **State**: The button is checkable. When checked (green background), the software FM demodulator is active on the current slice. When unchecked, it is disabled.
- **Interaction with mode changes**: Selecting any real radio mode from the Mode combo automatically deactivates the WFM overlay on that slice by emitting `wfmActivated(false)`. There is no "turn WFM on" branch from mode selection — you must click the WFM button to enable it.
- **Sync across applets**: If another applet or the core system changes the WFM state for this slice, `setWfmActive()` synchron