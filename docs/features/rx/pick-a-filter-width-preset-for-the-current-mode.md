...
## RX antenna selection improvements (v26.5.2.1, v26.7.4)
...
In v26.7.4, the RX antenna menu was further enhanced to integrate with the KiwiSDR feature. If a KiwiSDR manager is available, virtual antenna tokens from the KiwiSDR manager are appended to the menu options. The menu is now shown with `popup()` instead of `exec()`, and a triggered signal handler manages the selection. When a virtual antenna (associated with a KiwiSDR profile) is selected, the applet emits `kiwiRxAntennaSelected(sliceId, profileId)`. When a physical Flex antenna is selected, it emits `flexRxAntennaSelected(sliceId)` and calls `slice->setRxAntenna(token)`. The active KiwiSDR profile for the slice is shown as checked. The menu uses `QPointer<SliceModel>` to prevent use-after-free if the slice is destroyed while the menu is open.
...
In v26.6.3, a WFM toggle button was added next to the Mode combo in the