...
- **Gain-reduction bar**: Horizontal amber strip, right-filled. Scale maxes at 20 dB reduction; a tick at -6 dB marks a typical working amount. Refreshed ~30 Hz from `ClientComp::gainReductionDb()` with MeterSmoother ballistics (125 Hz animation, 30 ms attack / 180 ms release). The fill updates on every animation tick to maintain a consistent, responsive reading across all metering surfaces.
...
- The gain-reduction meter ballistics (125 Hz animation, 30 ms attack / 180 ms release) ensure the fill reads identically across all metering surfaces for consistent visual feedback.
