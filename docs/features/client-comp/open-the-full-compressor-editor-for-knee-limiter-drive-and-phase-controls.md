...
3. In the editor, you will see:
   ...
   - **Right column (TX only)**: Drive knob (0–18 dB pre-comp gain), Phase knob (0–6 all-pass stages), and a **Limiter** section with an **Enable** button and a Ceiling knob (sets the hard ceiling level).
...
| **Drive** (TX only, only in editor) | Pre-compression gain boost. Pushes more signal across the compressor threshold so the compressor works harder, raising average power. Pair with Phase to keep peaks clean. Persisted as `ClientCompTxDriveDb`. | 0.0 dB | 0.0 to 18.0 dB |
