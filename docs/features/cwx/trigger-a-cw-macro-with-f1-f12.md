...
- The TX slice must be in CW mode or one of the CW-derived modes. Function keys F1–F12 are silently ignored in all other modes.
- Each macro must have text stored in its slot. See [Edit a CW macro string](edit-a-cw-macro-string.md) if the slots are empty.

## Steps

1. Open the CWX panel. It appears automatically when the TX slice is in a CW mode.
...
|---|---|---|---|---|
| F1 … F12 (keyboard) | Sends the macro stored for that function key. Active when the TX slice is in a CW mode, regardless of CWX panel visibility. | — | — | `CwxMacro_F1` … `CwxMacro_F12` |
...
| F1 … F12 macro editors | Setup view editors for each macro. | — | — | `CwxMacro_F1` … `CwxMacro_F12` |
...
- The F1–F12 shortcuts are application-wide and enabled based on the TX slice's mode, not panel visibility. This means they work even if the CWX panel is minimized or another panel is visible, as long as the TX slice is in a CW mode. The shortcuts are mutually exclusive with the DVK panel's F1–F12 set (used in SSB/Digital modes) to avoid ambiguity.
...
- **Pressing F1–F12 does nothing** — Confirm the TX slice is in a CW mode. The shortcuts are disabled when a slice in SSB or Digital mode is active on the TX slice. If another application has captured the function keys, bring AetherSDR to the foreground.
