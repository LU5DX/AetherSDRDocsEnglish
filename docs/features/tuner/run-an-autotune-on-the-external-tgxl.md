...
| Fwd Pwr | Displays TGXL-reported forward power. Scale depends on your configuration: 0–200 W barefoot, 0–600 W Aurora, 0–2000 W with PGXL. The power scale is set automatically based on the connected radio and amplifier, and updates only when the relevant configuration changes. Gauge features ballistic behavior: bar rises quickly on RF bursts but decays over approximately 800 ms. Peak hold marker (white tick) appears at the highest forward power observed; clears after 2.5 seconds of no new peaks. When forward power is below 5 W, the gauge reads zero to prevent idle noise from lighting the bar. | 0–2000 W |
